...
_Proof_ - Let's consider a point $x^*$ which is a local minimizer of $f$. By definition of a local minimizer, there exists a radius $r > 0$ such that for all $x in RR^d$ such that $||x - x^*|| <= r$, we have $f(x^*) <= f(x)$.
This means that for $h in RR^d$ such that $||h|| <= r$, we have $f(x^*)
<= f(x^* + h) = f(x^*) + nabla f(x^*)^T h + o(||h||)$.
Simplifying by $f(x^*)$ and dividing by $||h||$ (which is non zero as $h$ is non zero), we obtain, $nabla f(x^*)^T bar(h) >= 0$, with $bar(h) = h / ||h||$.
We can apply the same reasoning with $-h$ instead of $h$ and obtain $nabla f(x^*)^T (-bar(h)) >= 0$, which gives $nabla f(x^*)^T bar(h) <= 0$.
Eventually, we have $nabla f(x^*)^T bar(h) = 0$ for all $bar(h)$ such that $||bar(h)|| = 1$, and thus $nabla f(x^*) = 0$.
#align(right)[$qed$]
